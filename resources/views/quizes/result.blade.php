                    ...
                    <div class="flex flex-col align-center">
                        ...
                        <div class="flex justify-between items-center space-x-4">
                            @if($userQuiz->question->type->type == 'meerkeuze')
                                ...
                                <h4 class="@if($userQuiz->is_correct) focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800 @else focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900 @endif">@if($userQuiz->is_correct) 1 @else 0 @endif</h4>
                            @elseif($userQuiz->question->type->type == 'open')
                                <h4 class="text-2xl italic my-5 ">{{$userQuiz->open_answer}}</h4>
                                <h4 class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900 ">Deze vraag wordt beoordeeld door de docent.</h4>
                            @endif
                        </div>
                    </div>
                </div>
